...
CREATE TABLE IF NOT EXISTS `reto_coordinadora`.`assistants` (
    `idassistants` VARCHAR(36) NOT NULL, `users_idusers` VARCHAR(36) NOT NULL, `events_idevents` VARCHAR(36) NOT NULL, `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `deleted_at` TIMESTAMP NULL, PRIMARY KEY (`idassistants`), INDEX `fk_assistants_users1_idx` (`users_idusers` ASC) VISIBLE, INDEX `fk_assistants_events1_idx` (`events_idevents` ASC) VISIBLE, CONSTRAINT `fk_assistants_users1` FOREIGN KEY (`users_idusers`) REFERENCES `reto_coordinadora`.`users` (`idusers`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_assistants_events1` FOREIGN KEY (`events_idevents`) REFERENCES `reto_coordinadora`.`events` (`idevents`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `reto_coordinadora`.`user_roles` (
    `iduser_roles` VARCHAR(36) NOT NULL, `roles_idroles` VARCHAR(36) NOT NULL, `users_idusers` VARCHAR(36) NOT NULL, `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `deleted_at` TIMESTAMP NULL, PRIMARY KEY (`iduser_roles`), INDEX `fk_user_roles_roles_idx` (`roles_idroles` ASC) VISIBLE, INDEX `fk_user_roles_users1_idx` (`users_idusers` ASC) VISIBLE, CONSTRAINT `fk_user_roles_roles` FOREIGN KEY (`roles_idroles`) REFERENCES `reto_coordinadora`.`roles` (`idroles`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_user_roles_users1` FOREIGN KEY (`users_idusers`) REFERENCES `reto_coordinadora`.`users` (`idusers`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB;
